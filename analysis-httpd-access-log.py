...
def run_batch_analysis():
    ...
    print("=" * 70)
    print("LogSentinelAI - HTTPD Access Log Analysis (Batch Mode)")
    print("=" * 70)
    ...
    # config = get_analysis_config("httpd_access", chunk_size=5)  # Override chunk_size
    config = get_analysis_config("httpd_access")  # Use default chunk_size
    ...
    with open(log_path, "r", encoding="utf-8") as f:
        for i, chunk in enumerate(chunked_iterable(f, chunk_size, debug=False)):
            ...
            prompt = PROMPT_TEMPLATE_HTTPD_ACCESS_LOG.format(logs=logs, model_schema=model_schema, response_language=response_language)
            print(f"\n--- Chunk {i+1} ---")
...
def run_realtime_analysis(chunk_size=None, log_path=None, processing_mode=None, sampling_threshold=None):
    ...
    print("=" * 70)
    print("LogSentinelAI - HTTPD Access Log Analysis (Real-time Mode)")
    print("=" * 70)
    ...
    def create_analysis_prompt(chunk, response_language):
        logs = "".join(chunk)
        model_schema = LogAnalysis.model_json_schema()
        return PROMPT_TEMPLATE_HTTPD_ACCESS_LOG.format(
            logs=logs, 
...
def main():
    ...
    parser = argparse.ArgumentParser(description='HTTPD Access Log Analysis')
